# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 15:56:04 2020
...
def Hbeta(D = np.array([]), beta = 1.0):
    ...
    # compute P-row and corresponding complexity
...
def x2p(X = np.array([]), tol = 1e-05, perplexity = 30.0):
    ...
    D = np.add(np.add(-2 * np.dot(X, X.T), sum_X).T, sumX)
    P = np.zeros((n, n))
    beta = np.ones((n, 1))
...
def tsne(X = np.array([]), no_dims = 2, initial_dims = 50, perplexity = 30.0,
    ...
    Added by F.Marquardt (sexy): do_animation == True will give you a graphicalanimation of
