# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 09:45:59 2020
...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, 
    ...
    for iter in range(max_iter):
        ...
        num = -2. * np.dot(Y, Y.T)           # Correlation matrix
        num = 1. / (1. + np.add(np.add(num, sum_Y).T, sum_Y)) 
        num[range(n), range(n)] = 0. # Elimination of the diagonal entries
        Q = num / np.sum(num)
...
##############################################################################
#### EXAMPLE: REPRODUCING BOTH RANDOMLY PLACED SQUARES #######################
#### AND ROTATED SQUARES WITH THE SAME AUTOENCODER ###########################
##############################################################################
...
vals=np.linspace(-1,1,M)
x,y=np.meshgrid(vals,vals)
test_input = np.zeros([batchsize, M, M, 1])
test_input[:,:,:,0] = my_generator_squares_rotated(batchsize,x,y)
